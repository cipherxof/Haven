...
public class Geom
{
    ...
    public Geom(EndianBinaryReader reader)
    {
        var pos = reader.BaseStream.Position;
        Offset = checked((int)pos);

        Length = reader.ReadByte();
        Type = reader.ReadByte();
        Field002 = reader.ReadByte();
        Field003 = reader.ReadByte();
        Flag = (uint)(Length << 24 | Type << 16 | Field002 << 8 | Field003);
